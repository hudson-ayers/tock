...
pub enum UartChannel<'a> {
    Pins(UartPins),
    Rtt(crate::segger_rtt::SeggerRttMemoryRefs<'a>),
}
    ...
    unsafe fn finalize(self, _s: Self::StaticInput) -> Self::Output {
        match self.uart_channel {
            ...
            UartChannel::Rtt(rtt_memory) => {
                let rtt = crate::segger_rtt::SeggerRttComponent::new(self.mux_alarm, rtt_memory)
                    .finalize(crate::segger_rtt_component_helper!(nrf52::rtc::Rtc));
                rtt
            }
        }
    }
